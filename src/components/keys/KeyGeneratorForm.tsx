...
const MARKETPLACE_PRODUCTS_UPDATED = 'marketplace:products-updated';
...
export function KeyGeneratorForm({ products, onGenerate, submitting }: KeyGeneratorFormProps) {
  ...
  // ✅ ADD: Track product list updates
  ...
  // ✅ ADD: Listen for product list updates
  useEffect(() => {
    const handleProductsUpdate = (event: Event) => {
      const customEvent = event as CustomEvent;
      console.log('[KeyGeneratorForm] Products list updated');
      
      // Refresh the product list by incrementing key
      // This forces parent re-render if needed
      setRefreshKey(prev => prev + 1);
      
      // Clear product selection if current product no longer exists
      if (productId && !products.find(p => p.id === productId)) {
        setProductId('');
        toast.info('Selected product no longer available');
      }
    };

    window.addEventListener(MARKETPLACE_PRODUCTS_UPDATED, handleProductsUpdate);
    
    return () => {
      window.removeEventListener(MARKETPLACE_PRODUCTS_UPDATED, handleProductsUpdate);
    };
  }, [productId, products]);
  ...
  return (
    <div className="glass-card rounded-xl p-6 space-y-6" key={refreshKey}>
