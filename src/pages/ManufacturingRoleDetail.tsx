 ...
 const techStack = [
   ...
   { name: 'BACKEND', icon: Server, tech: manufacturingTechStack[1].tech },
   { name: 'DATABASE', icon: Database, tech: manufacturingTechStack[2].tech },
   { name: 'SERVER / CLOUD', icon: Cloud, tech: manufacturingTechStack[3].tech },
   { name: 'AI ENGINE', icon: Brain, tech: manufacturingTechStack[4].tech },
   { name: 'SECURITY LAYER', icon: Shield, tech: manufacturingTechStack[5].tech },
 ];
 
 export default function ManufacturingRoleDetail() {
   const [enabledCategories, setEnabledCategories] = useState<Record<number, boolean>>(
     Object.fromEntries(manufacturingSubCategories.map(c => [c.id, true]))
   );
   const [permissions, setPermissions] = useState<Record<number, Record<string, boolean>>>(
     Object.fromEntries(manufacturingSubCategories.map(c => [c.id, { view: true, add: true, edit: true, delete: false, export: true }]))
   );
   const [integrations, setIntegrations] = useState<Record<number, Record<string, boolean>>>(
     Object.fromEntries(manufacturingSubCategories.map(c => [c.id, { payment: true, sms: true, whatsapp: false, ai: true, thirdParty: false }]))
   );
             ...
             <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
               <div>
                 <h2 className="text-2xl md:text-3xl font-bold text-foreground uppercase tracking-wide">
                   {manufacturingRole.name}
                 </h2>
                 <p className="text-muted-foreground mt-2">
                   {manufacturingRole.purpose}
                 </p>
               </div>
               <div className="flex flex-wrap items-center gap-3">
                 <Badge className={cn(
                   "font-bold",
                   manufacturingRole.status === 'active' 
                     ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30"
                     : "bg-red-500/20 text-red-400 border-red-500/30"
                 )}>
                   {manufacturingRole.status.toUpperCase()}
                 </Badge>
                 <Badge variant="outline" className="font-semibold">
                   {manufacturingRole.version}
                 </Badge>
                 <span className="text-xs text-muted-foreground">
                   Updated: {manufacturingRole.lastUpdated}
                 </span>
               </div>
             </div>
           </div>
           ...
           <div className="bg-card border border-border rounded-2xl p-6">
             <div className="flex items-center justify-between mb-6">
               <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wide">
                 SUB-CATEGORIES ({manufacturingSubCategories.length})
               </h3>
               <Badge variant="outline" className="font-semibold">
             ...
             <Accordion type="single" collapsible className="space-y-3">
               {manufacturingSubCategories.map((category) => (
                 <AccordionItem 
                   key={category.id} 
