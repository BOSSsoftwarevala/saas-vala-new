...
const PRODUCTS = [
  { id: 'trans-pwa-1', name: 'Uber Clone', repo: 'https://github.com/saasvala/uber-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Ride-hailing platform with driver dashboard and GPS tracking.', features: ['Ride Booking', 'Driver Dashboard', 'GPS Tracking', 'Payment Integration', 'Mobile App'] },
  { id: 'trans-pwa-2', name: 'Lyft Clone', repo: 'https://github.com/saasvala/lyft-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Ride booking with driver management and route optimization.', features: ['Ride Booking', 'Driver Management', 'Route Optimization', 'Payment Gateway', 'Mobile App'] },
  { id: 'trans-pwa-3', name: 'Grab Clone', repo: 'https://github.com/saasvala/grab-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Multi-service ride booking with fleet management and payments.', features: ['Ride Booking', 'Driver Dashboard', 'Fleet Management', 'Payment Integration', 'Mobile App'] },
  { id: 'trans-pwa-4', name: 'Ola Clone', repo: 'https://github.com/saasvala/ola-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Ride-hailing with GPS tracking, driver dashboard, and payments.', features: ['Ride Booking', 'GPS Tracking', 'Driver Dashboard', 'Payment Gateway', 'Mobile App'] },
  { id: 'trans-pwa-5', name: 'Bolt Clone', repo: 'https://github.com/saasvala/bolt-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Affordable ride booking with route optimization and driver tools.', features: ['Ride Booking', 'Driver Management', 'Route Optimization', 'Mobile App', 'Payment Integration'] },
];

const VALID_KEYS = ['TRANS-PWA-2026-001', 'TRANS-PWA-2026-002', 'TRANS-PWA-2026-003', 'TRANS-APK-2026-001'];
const PFX = 'trans-pwa';
...
export default function TransportPwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 Transportation & Mobility software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Transportation & Mobility Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-transport-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Transportation & Mobility — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Transportation & Mobility Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: TRANS-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Transportation Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🚗" title="Transportation & Mobility" subtitle="Top 5 Transportation & Mobility Software Clones — Offline Ready." badge="ROW 12" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Transportation & Mobility software demos offline.</p>
            <Input placeholder="TRANS-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
