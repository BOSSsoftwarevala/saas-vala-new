...
const PRODUCTS = [
  { id: 'fin-pwa-1', name: 'NetBank Clone', repo: 'https://github.com/saasvala/netbank-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Online banking platform with account management and transactions.', features: ['Online Banking', 'Account Management', 'Transaction History', 'Payment Gateway', 'Mobile App'] },
  { id: 'fin-pwa-2', name: 'BankBazaar Clone', repo: 'https://github.com/saasvala/bankbazaar-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Loan and insurance marketplace with account management.', features: ['Loan Management', 'Insurance Portal', 'Account Management', 'Payment Gateway', 'Mobile Dashboard'] },
  { id: 'fin-pwa-3', name: 'Paytm Clone', repo: 'https://github.com/saasvala/paytm-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Mobile wallet with payments, banking, and bill payments.', features: ['Mobile Wallet', 'Payments', 'Banking Services', 'Bill Payments', 'Mobile App'] },
  { id: 'fin-pwa-4', name: 'Razorpay Clone', repo: 'https://github.com/saasvala/razorpay-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Payment gateway with invoice management and analytics.', features: ['Payment Gateway', 'Invoice Management', 'Transaction Tracking', 'Mobile App', 'Analytics Dashboard'] },
  { id: 'fin-pwa-5', name: 'PolicyBazaar Clone', repo: 'https://github.com/saasvala/policybazaar-clone-software', price: 5, old_price: 10, rating: 4.9, description: 'Insurance comparison portal with premium calculator.', features: ['Insurance Portal', 'Premium Calculator', 'Policy Management', 'Customer Dashboard', 'Mobile App'] },
];

const VALID_KEYS = ['FINANCE-PWA-2026-001', 'FINANCE-PWA-2026-002', 'FINANCE-PWA-2026-003', 'FINANCE-APK-2026-001'];
const PFX = 'fin-pwa';
...
export default function FinancePwa() {
  ...
  const activate = () => {
    if (VALID_KEYS.includes(keyInput.trim().toUpperCase())) {
      ...
      toast.success('🎉 License activated! All 5 Finance, Banking & Insurance software demos unlocked.');
    } else toast.error('Invalid license key.');
  };

  const download = () => {
    if (!activated) { setShowKey(true); return; }
    const blob = new Blob([JSON.stringify({ bundle: 'SaaS VALA Finance, Banking & Insurance Master Copy', version: '2026.1', activated: true, products: PRODUCTS.map(p => ({ name: p.name, repo: p.repo, features: p.features })), generatedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const u = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = u; a.download = 'saas-vala-finance-master-copy.json'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border px-4 md:px-8 py-4 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-primary">SaaS VALA</h1><p className="text-xs text-muted-foreground">Finance, Banking & Insurance — Offline PWA</p></div>
      ...
      <main className="py-6 space-y-6">
        {!activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-primary/30 bg-primary/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><Lock className="h-5 w-5 text-primary" /><div><p className="font-semibold text-sm">Activate to unlock all 5 Finance, Banking & Insurance Software Demos</p><p className="text-xs text-muted-foreground">Enter license key: FINANCE-PWA-2026-001</p></div></div>
        ...
        {activated && (
          <div className="mx-4 md:mx-8 p-4 rounded-lg border border-green-500/30 bg-green-500/5 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-500" /><div><p className="font-semibold text-sm">Master Copy Ready — All 5 Finance Software Unlocked</p><p className="text-xs text-muted-foreground">Download the complete offline bundle</p></div></div>
        ...
        <SectionHeader icon="🏦" title="Finance, Banking & Insurance" subtitle="Top 5 Finance, Banking & Insurance Software Clones — Offline Ready." badge="ROW 14" badgeVariant="hot" totalCount={5} />
        <SectionSlider>
          {PRODUCTS.map((p, i) => (
            <div key={p.id} className="min-w-[280px] max-w-[320px] flex-shrink-0 group">
              <Card className="relative overflow-hidden border-border/50 bg-card hover:border-primary/40 transition-all duration-300 hover:scale-[1.05] hover:shadow-[0_0_30px_rgba(249,115,22,0.15)]">
                ...
                <CardContent className="p-4 space-y-3">
                  <Badge variant="outline" className="text-[9px] uppercase tracking-widest text-primary border-primary/30">Finance</Badge>
      ...
      <Dialog open={showKey} onOpenChange={setShowKey}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-2">
            <p className="text-sm text-muted-foreground">Enter your license key to unlock all 5 Finance, Banking & Insurance software demos offline.</p>
            <Input placeholder="FINANCE-PWA-2026-001" value={keyInput} onChange={e => setKeyInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && activate()} className="font-mono text-center tracking-widest" />
